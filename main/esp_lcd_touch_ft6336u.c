...
esp_err_t esp_lcd_touch_new_i2c_ft6336u(const esp_lcd_panel_io_handle_t io, const esp_lcd_touch_config_t *config, esp_lcd_touch_handle_t *tp)
{
...
static esp_err_t read_data(esp_lcd_touch_handle_t tp)
{
    ...
    // 0x02: Num of touch + Touch1 XH, XL, YH, YL
    ESP_RETURN_ON_ERROR(i2c_read_bytes(tp, DATA_START_REG, data, sizeof(data)), TAG, "I2C read failed");

    uint8_t touches = data[0] & 0x0F;  // FT6336U 的觸控點數在低4位元

    portENTER_CRITICAL(&tp->data.lock);
    tp->data.points = (touches > POINT_NUM_MAX ? POINT_NUM_MAX : touches);

    if (tp->data.points > 0) {
        tp->data.coords[0].x = ((data[1] & 0x0F) << 8) | data[2];  // XH (bit[3:0]) + XL
        tp->data.coords[0].y = ((data[3] & 0x0F) << 8) | data[4];  // YH (bit[3:0]) + YL
        tp->data.coords[0].strength = 255; // FT6336U 無壓力值, 給固定值
    }
