...
esp_err_t read_from_HS4011(i2c_device_t *device, HS4011_DATA *results){
    ...
    ret = i2c_master_transmit(device->dev_handle,write_buf,sizeof(write_buf), 50);
    if (ret != ESP_OK){
    ...
    ret = i2c_master_receive(device->dev_handle, read_buf, sizeof(read_buf), 50);
    if (ret != ESP_OK) {
...
esp_err_t set_HS4011_mode(i2c_device_t const *device){
    esp_err_t ret = ESP_OK;
    uint8_t write_buf[1] = {0xFE}; // Fistly make it into sleep mode, and then we can adjust parms

    ret = i2c_master_transmit(device->dev_handle, write_buf, sizeof(write_buf), 200);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "Failed to reset HS4011");
        return ret;
    }
    vTaskDelay(pdMS_TO_TICKS(10));
...
esp_err_t set_BME280_mode(i2c_device_t const *device){
    ...
    ret = i2c_master_transmit(device->dev_handle, write_buf2, sizeof(write_buf2), 50);
    if (ret != ESP_OK) {
    ...
    ret = i2c_master_transmit(device->dev_handle, write_buf3, sizeof(write_buf3), 50);
    if (ret != ESP_OK) {
    ...
    ret = i2c_master_transmit(device->dev_handle, write_buf4, sizeof(write_buf4), 50);
    if (ret != ESP_OK) {
...
esp_err_t read_from_BME280(i2c_device_t *device, BME280_DATA *results){
    ...
    int retries = 3;
    while (retries>0){
        ret = i2c_master_transmit_receive(device->dev_handle,write_buf,sizeof(write_buf),read_buf,sizeof(read_buf),100);
        if (ret == ESP_OK) {
            break;
        }
        retries--;
    ...
    retries = 3;
    while (retries>0){
        ret = i2c_master_receive(device->dev_handle, read_buf2, sizeof(read_buf2), 50);
        if (ret == ESP_OK) {
            break;
        }
        retries--;
    ...
    int32_t P_raw = ((int32_t)read_buf2[0] << 12) | ((int32_t)read_buf2[1] << 4) | (read_buf2[2] >> 4);
...
esp_err_t read_from_MEMS(i2c_device_t *multiplexer,i2c_device_t *devices, MEMS_DATA *results){
    ...
    for (int i = 0; i < 8; i++){  
        ...
        uint8_t write_buf[1] = {CH0+i}; 
        uint8_t read_buf_RS_L[1],read_buf_RS_H[1],read_buf_RS_OF[1];
        ret = i2c_master_transmit(multiplexer->dev_handle, write_buf, sizeof(write_buf), 20);
        if (ret != ESP_OK) {
        ...
        // i2c_master_transmit(devices->dev_handle, write_buf1, sizeof(write_buf1), 100);
        // if (ret != ESP_OK) {
        //     ESP_LOGE(TAG, "Failed to write register address 0x04");
        //     return ret;
        // }

        // ret = i2c_master_receive(devices->dev_handle, read_buf_RS_L, sizeof(read_buf_RS_L), 100);
        // if (ret != ESP_OK) {
        //     ESP_LOGE(TAG, "Failed to read data from 0x04");
        //     return ret;
        // }

        ret = i2c_master_transmit_receive(devices->dev_handle,write_buf1, sizeof(write_buf1),read_buf_RS_L, sizeof(read_buf_RS_L),100);
        if (ret != ESP_OK) {
        ...
        ret = i2c_master_transmit_receive(devices->dev_handle,write_buf2, sizeof(write_buf2),read_buf_RS_H, sizeof(read_buf_RS_H),100);
        if (ret != ESP_OK) {
        ...
        ret = i2c_master_transmit_receive(devices->dev_handle,write_buf3, sizeof(write_buf3),read_buf_RS_OF, sizeof(read_buf_RS_OF),100);
        if (ret != ESP_OK) {
        ...
        uint32_t resistant = ((uint32_t)read_buf_RS_H[0] << 8) | (uint32_t)read_buf_RS_L[0]; 

        if (read_buf_RS_OF[0] & 0x80) {
            // printf("Ch%d overflow\n",i);
            resistant = resistant<<6;
        }

        *((uint32_t*)(&results->ch1) + i) = resistant;
        // printf("ch%d: %ld\n",i,resistant);
    }
